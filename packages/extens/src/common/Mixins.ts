...
function getAllMemberKeysAsSet<T extends Object | Function>(thing: T, startLevel: number = 0, maxLevel?: number): Set<string | symbol> {
    const props: Set<string | symbol> = new Set<string | symbol>();
    if (thing === null || thing === undefined) {
        return props;
    }
    ...
    while (currentLevel <= maxLevel && currentObj !== undefined) {
        ...
        const _currentObj: T | ObjectConstructor = (currentObj === null)
            ? Object
            : currentObj;
...
function getProtoypeChain<T>(thing?: T, target?: T, levelLimit?: number): string {
 ...
 */
function inspect<T>(thing?: T): string {
    ...
    if (thing === undefined) {
        ...
        const prefix = stdBuiltIn.includes(parts[1] || '') ? 'Built-in' : 'Custom';
