...
import { makeConfig, ResourceLocation, SourceType, LangType } from "../../eslint.config.mjs";
...
const theUsualSuspects = [
    ...
    // but for this vs code extension building workspace, also:
    '.debug',
    '.vscode-test',
    'test',
    'out',
...
const configs = [
    // src / <esm|cjs> / ts
    makeConfig(ResourceLocation.src.id, SourceType.module.id, LangType.ts.id, ['src'], ['dist', ...theUsualSuspects], false),
    makeConfig(ResourceLocation.src.id, SourceType.commonjs.id, LangType.ts.id, ['src'], ['dist', ...theUsualSuspects], false),

    // src / <esm|cjs> / js
    makeConfig(ResourceLocation.src.id, SourceType.module.id, LangType.js.id, ['src'], ['dist', ...theUsualSuspects], false),
    makeConfig(ResourceLocation.src.id, SourceType.commonjs.id, LangType.js.id, ['src'], ['dist', ...theUsualSuspects], false),

    // dist / <esm|cjs> / js
    makeConfig(ResourceLocation.dist.id, SourceType.module.id, LangType.js.id, ['dist'], ['src', ...theUsualSuspects], false),
    makeConfig(ResourceLocation.dist.id, SourceType.commonjs.id, LangType.js.id, ['dist'], ['src', ...theUsualSuspects], false),
];
